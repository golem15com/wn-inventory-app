...
trait Financial
{
    ...
    /**
     * Per-class boot guard. Prevents duplicate boot of the trait for the same class
     * while allowing each class that uses the trait to boot exactly once.
     *
     * Replaces the former single shared boolean flag which caused the SECOND class
     * using the trait to skip boot entirely -- leaving model.beforeSetAttribute
     * UNBOUND for that class. See OTF-001 / PAY-13.
     *
     * @var array<class-string, true>
     */
    protected static $financialTraitBootedFor = [];
    ...
    {
        $class = static::class;
        if (isset(static::$financialTraitBootedFor[$class])) {
            return;
        }
        static::$financialTraitBootedFor[$class] = true;
